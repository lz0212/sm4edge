...
"\n");
        ...
        printf(" -A1               | disable payload encryption, do not use with key, defaults\n"
               "                   | to AES then\n");
        printf(" -A2 ... -A6       | choose a cipher for payload encryption, requires a key,\n"
               "                   | -A2 = Twofish, -A3 = AES (default if key provided),\n"
               "                   | -A4 = ChaCha20, -A5 = Speck-CTR, -A6 = SM4\n");
...
int main (int argc, char* argv[]) {
    ...
    if(conf.transop_id == N2N_TRANSFORM_ID_NULL) {
        if(conf.encrypt_key) {
            // make sure that SM4 is default cipher if key only (and no cipher) is specified
            traceEvent(TRACE_WARNING, "switching to SM4 as key was provided");
            conf.transop_id = N2N_TRANSFORM_ID_SM4;
        }
    }
    // user auth

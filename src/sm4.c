...
int ossl_sm4_set_key(const uint8_t *key, sm4_context_t *ctx)
{
    /*
     * Family Key密钥扩展算法 系统参数
     */
...
void ossl_sm4_encrypt(const uint8_t *in, uint8_t *out, sm4_context_t *ctx)
{
...
void ossl_sm4_decrypt(const uint8_t *in, uint8_t *out, sm4_context_t *ctx)
{
    uint32_t B0 = load_u32_be(in, 0);
    uint32_t B1 = load_u32_be(in, 1);
    uint32_t B2 = load_u32_be(in, 2);
...
int sm4_cbc_encrypt (unsigned char *out, const unsigned char *in, size_t in_len,
    ...
    n = in_len / SM4_BLOCK_SIZE;
    for(i=0; i < n; i++) {
        fix_xor(tmp, &in[i * SM4_BLOCK_SIZE]);
        ossl_sm4_encrypt(tmp, tmp, ctx);
        memcpy(&out[i * SM4_BLOCK_SIZE], tmp, SM4_BLOCK_SIZE);
    }

    return n * SM4_BLOCK_SIZE;
}
...
int sm4_cbc_decrypt (unsigned char *out, const unsigned char *in, size_t in_len,
    ...
    n = in_len / SM4_BLOCK_SIZE;
    for(i=0; i < n; i++) {
        memcpy(old, &in[i * SM4_BLOCK_SIZE], SM4_BLOCK_SIZE);
        ossl_sm4_decrypt(&in[i * SM4_BLOCK_SIZE], &out[i * SM4_BLOCK_SIZE], ctx);
        fix_xor(&out[i * SM4_BLOCK_SIZE], tmp);
        memcpy(tmp, old, SM4_BLOCK_SIZE);
    }
...
int sm4_init (const unsigned char *key, sm4_context_t **ctx)
 {
    ...
    *ctx = calloc(1, sizeof(sm4_context_t));
    if(!(*ctx)) {
        return -1;
    }
    ossl_sm4_set_key(key, *ctx);
